...
public class AnalysisSettingsPreferencePage extends PreferencePage {
	...
	protected Control createContents(Composite parent) {
		...
		dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(textDuration), BeanProperties.value(IAnalysis.class, IAnalysis.PROPERTY_DURATION).observe(analysis), new UpdateValueStrategy(UpdateValueStrategy.POLICY_CONVERT).setAfterConvertValidator(new ValidatorDuration()).setConverter(new MinutesToMilliseconds()), new UpdateValueStrategy().setConverter(new MillisecondsToMinutes()));
